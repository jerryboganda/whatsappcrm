    ...
    <div class="dashboard-container">
        <div class="container-top">
            <div class="container-top__left">
                ...
                <p class="container-top__desc"><?php echo app('translator')->get('Start your next campaign by completing the form below.'); ?></p>
            </div>
            <div class="container-top__right">
                <div class="btn--group">
                    <a href="<?php echo e(route('user.campaign.index')); ?>" class="btn btn--dark">
                        <i class="las la-undo"></i>
                        <?php echo app('translator')->get('Back'); ?>
                    </a>
                    <button type="submit" form="campaign-form" class="btn btn--base btn-shadow">
                        <i class="lab la-telegram"></i> <?php echo app('translator')->get('Save Campaign'); ?>
                    </button>
                </div>
            </div>
        </div>
        <div class="dashboard-container__body">
            <div class="information-wrapper">
                <form action="<?php echo e(route('user.campaign.save')); ?>" method="POST" class="information-main-form"
                    id="campaign-form">
                    <?php echo csrf_field(); ?>
                    <div class="row gy-2">
                        <div class="col-12">
                            <div class="form-group">
                                <label class="label-two"><?php echo app('translator')->get('Title'); ?></label>
                                <input type="text" class="form--control form-two" name="title"
                                    placeholder="<?php echo app('translator')->get('Enter campaign title'); ?>" required value="<?php echo e(old('title')); ?>">
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="form-group">
...
<?php endif; ?>
                                <span class="fs-12 text-dark">
                                    <i><?php echo app('translator')->get('Please select the WhatsApp account from which the message will be sent or the campaign will be started'); ?></i>
                                </span>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="form-group">
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    <label class="label-two mb-0"><?php echo app('translator')->get('Contact From List'); ?></label>
                                    <a href="<?php echo e(route('user.contact.list', ['open_group_import' => 1])); ?>" target="_blank"
                                        class="btn btn--sm btn--info">
                                        <i class="lab la-whatsapp"></i> <?php echo app('translator')->get('Import from WhatsApp Group'); ?>
                                    </a>
                                </div>
                                <select name="contact_lists[]" class="form--control select2 form-two contactList"
                                    ...
                                    <?php $__currentLoopData = $contactLists; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $list): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($list->id); ?>" <?php if(in_array($list->id, old('contact_lists', []))): echo 'selected'; endif; ?>>
                                            <?php echo e(__(@$list->name) . ' (' . $list->contact->count() . ')'); ?>
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                <label class="label-two"><?php echo app('translator')->get('Contact From Tag'); ?></label>
                                <select name="contact_tags[]" class="form--control select2 form-two contactTag"
                                    ...
                                    <?php $__currentLoopData = $contactTags; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $contactTag): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <option value="<?php echo e($contactTag->id); ?>" <?php if(in_array($contactTag->id, old('contact_tags', []))): echo 'selected'; endif; ?>>
                                            <?php echo e(__(@$contactTag->name) . ' (' . $contactTag->contacts->count() . ')'); ?>
                        ...
                        <div class="col-md-12" id="schedule">
                            <div class="form-group">
                                <label class="label-two"><?php echo app('translator')->get('Send At'); ?></label>
                                <select name="schedule" class="form--control form-two select2"
                                    data-minimum-results-for-search="-1" required>
                                    <option value="off"><?php echo app('translator')->get('Send Now'); ?></option>
                                    <option value="on" selected><?php echo app('translator')->get('Schedule At'); ?></option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6 d-none" id="date">
                            <div class="form-group">
                                <label class="label-two"><?php echo app('translator')->get('Scheduled At'); ?></label>
                                <input type="date" class="form--control form-two date" name="scheduled_at">
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="form-group">
                                <label class="label-two"><?php echo app('translator')->get('Choose template'); ?></label>
                                <select name="template_id" class="form--control select2 form-two"
                                    data-minimum-results-for-search="-1" required>
                                    <option value="" disabled><?php echo app('translator')->get('Please select whatsapp account first'); ?></option>
                                </select>
                            </div>
                        </div>
    ...
    <script>
        (function($) {
            ...
            $('#campaign-form').on("submit", function(e) {
                e.preventDefault();
                let form = $(this);
                let route = "<?php echo e(route('user.campaign.save')); ?>";
            ...
            function getWhatsappAccountTemplates() {
                ...
                let route = "<?php echo e(route('user.template.get', ':id')); ?>";
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('select[name=template_id]').append(
                            `<option value="" selected><?php echo app('translator')->get('Select template'); ?></option>`);
                        let templates = response.data.templates ?? [];
                        if (templates.length != 0) {
                            $('select[name=template_id]').empty();
                        };
                        let headerBasePath = "<?php echo e(asset(getFilePath('templateHeader'))); ?>";
            ...
            function showTemplatePreview() {
                ...
                if (headerFormat === 'IMAGE' && headerMediaPath) {
                    ...
                    if (templateHeaderText == null) {
                        ...
                        $headerText.text(templateHeaderText).removeClass('d-none');
                    }
                }
            }
...
<?php echo $__env->make($activeTemplate . 'layouts.master', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/core/resources/views/templates/basic/user/campaign/create.blade.php ENDPATH**/ ?>