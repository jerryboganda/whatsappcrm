...
<?php endif; ?>
            <div class="search-form">
                <input class="form--control conversation-search" name="search" type="text"
                    placeholder="<?php echo app('translator')->get('Search conversation'); ?>..." autocomplete="off">
            ...
            <div class="chatbox-wrapper__tab">
                <ul class="nav nav-pills custom--tab tab-two" id="chat-filters">
                    <li class="nav-item">
                        <button class="nav-link <?php echo e(activeClass(!request()->status && !request()->filter)); ?>"
                            data-status="0"><?php echo app('translator')->get('All'); ?>
                        </button>
                    </li>
                    <li class="nav-item">
                        <button class="nav-link <?php echo e(activeClass(request()->filter == 'mine')); ?>"
                            data-filter="mine"><?php echo app('translator')->get('Mine'); ?>
                        </button>
                    </li>
                    <li class="nav-item">
                        <button class="nav-link <?php echo e(activeClass(request()->filter == 'unassigned')); ?>"
                            data-filter="unassigned">
                            <?php echo app('translator')->get('Unassigned'); ?>
                        </button>
                    </li>
                    <li class="nav-item">
                        <button class="nav-link <?php echo e(activeClass(request()->status == Status::UNREAD_CONVERSATION)); ?>"
                            data-status="<?php echo e(Status::UNREAD_CONVERSATION); ?>">
                            <?php echo app('translator')->get('Unread'); ?>
                        </button>
                    </li>
                    <li class="nav-item">
                        <button class="nav-link <?php echo e(activeClass(request()->status == Status::PENDING_CONVERSATION)); ?>"
                            data-status="<?php echo e(Status::PENDING_CONVERSATION); ?>">
                            <?php echo app('translator')->get('Pending'); ?>
                        </button>
                    </li>
                    <li class="nav-item">
                        <button class="nav-link <?php echo e(activeClass(request()->status == Status::DONE_CONVERSATION)); ?>"
                            data-status="<?php echo e(Status::DONE_CONVERSATION); ?>">
                            <?php echo app('translator')->get('Done'); ?>
                        </button>
                    </li>
                    <li class="nav-item">
                        <button class="nav-link <?php echo e(activeClass(request()->status == Status::IMPORTANT_CONVERSATION)); ?>"
                            data-status="<?php echo e(Status::IMPORTANT_CONVERSATION); ?>">
...
<?php $__env->startPush('script'); ?>
    <script>
        ...
        (function ($) {
            ...
            let status = "<?php echo e(request()->status ?? 0); ?>";
            let filter = "<?php echo e(request()->filter ?? ''); ?>";

            window.conversation_id = "<?php echo e($conversationId); ?>";
            window.whatsapp_account_id = "<?php echo e(request()->whatsapp_account_id ?? 0); ?>";
            ...
            window.fetchChatList = function (search = '', resetPage = false) {
                ...
                let url = "<?php echo e(route('user.inbox.conversation.list')); ?>";
                $.ajax({
                    url: `${url}?page=${page}`,
                    method: 'GET',
                    data: {
            ...
            function conversationSkeleton() {
                ...
                let html = `<div class="conversation-loader text-center d-flex align-items-center justify-content-center flex-column  ${page == 1 ? 'h-50vh' : 'my-5'}">
                                ...
                                ${page == 1 ? `<p class="fs-16 mt-1"><?php echo app('translator')->get('Conversation is Loading'); ?>...</p>` : ''}
                                </div>`
            ...
            function messageLoader() {
                ...
                let html = `<div class="message-loader text-center h-100 d-flex align-items-center justify-content-center flex-column py-4">
                                    <div class="spinner-border text--base" role="status"></div>
                                ${messagePage == 1 ? `<p class="fs-16 mt-1"><?php echo app('translator')->get('Message is Loading'); ?>...</p>` : ''}
                                </div>`
            ...
            function errorHtml() {
                return ` <div class="error-message server-error-message text-center d-flex justify-content-center align-items-center flex-column gap-1 h-100">
                                        <img src="<?php echo e(asset($activeTemplateTrue . 'images/server_error.png')); ?>" alt="empty">
                                        <p class="fs-14"><?php echo app('translator')->get('Something went wrong. Please try later'); ?></p>
                                    </div>`
            }
            ...
            function loadMessages(search = '',) {
                ...
                let url = "<?php echo e(route('user.inbox.conversation.message', ':id')); ?>" + `?page=${messagePage}`;
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.status == 'success') {
                            ...
                            const mobileNumber = '<?php echo e($viewMobile); ?>' ? `+${mobile_code + mobile}` :
                                `${maskMobile}`;
            ...
            function loadContact() {
                const url = "<?php echo e(route('user.inbox.contact.details', ':id')); ?>";
                $.ajax({
                    url: url.replace(':id', window.conversation_id),
                    method: 'GET',
                    beforeSend: function () {
                    ...
                    success: function (response) {
                        if (response.status == 'success') {
                            ...
                            if (response.data.isBlocked == '<?php echo e(Status::YES); ?>') {
            ...
            $('.contact__details').on('change', ".statusForm select[name=conversation_status]", function () {
                let value = $(this).val();
                let route = "<?php echo e(route('user.inbox.conversation.status', ':id')); ?>";
            ...
            $('select[name=whatsapp_account_id]').on('change', function () {
                const id = $(this).val();
                const url = "<?php echo e(route('user.inbox.list')); ?>?whatsapp_account_id=" + id;
                window.location = url;
            });
            ...
            $('.assign-agent').on('change', function () {
                ...
                let url = "<?php echo e(route('user.inbox.conversation.assign', ':id')); ?>";
...
<?php $__env->stopPush(); ?><?php /**PATH /var/www/html/core/resources/views/templates/basic/user/inbox/conversation.blade.php ENDPATH**/ ?>