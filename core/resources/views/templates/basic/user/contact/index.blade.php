                ...
                <div class="container-top__right">
                    <div class="btn--group">
                        ...
                        <button type="button" class="btn btn--primary btn-shadow importGroupBtn">
                            ...
                            @lang('Import from WhatsApp Group')
                        </button>
                    </div>
                </div>
            </x-permission_check>
    ...
    <div class="modal custom--modal fade importGroupModal">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <div class="modal-header__left">
                        <h5 class="title">@lang('Import from WhatsApp Group')</h5>
                        <p class="text">@lang('Use extension import for one-click extraction, or legacy paste as fallback.')</p>
                    </div>
                    ...
                    <div class="tab-content pt-3">
                        <div class="tab-pane fade show active" id="extension-import-pane" role="tabpanel">
                            ...
                            <div class="row g-3">
                                <div class="col-lg-5">
                                    <div class="form-group">
                                        <label class="label-two">@lang('Contact List')</label>
                                        <select class="form--control select2" id="geContactList" style="width: 100%">
                                            <option value="">@lang('Auto-create list')</option>
                                            @foreach ($contactLists as $list)
                                                <option value="{{ $list->id }}">{{ __(@$list->name) }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        <label class="label-two">@lang('Country Hint')</label>
                                    ...
                                    <div class="form-group">
                                        <label class="d-flex align-items-start gap-2">
                                            ...
                                            <span>
                                                @lang('I confirm I am authorized to extract and import these contacts into CRM.')
                                            </span>
                                        </label>
                                    </div>
                                    ...
                                    <div class="d-flex gap-2 flex-wrap">
                                        ...
                                        @endphp
                                        <a href="{{ asset('extensions/wa-group-capture/wa-group-capture.zip') }}?v={{ $groupCaptureZipVersion }}"
                                            class="btn btn--dark btn--sm" target="_blank">
                                            <i class="las la-download"></i> @lang('Download Extension')
                                        </a>
                                        <button type="button" class="btn btn--base btn--sm" id="geConnectBtn">
                                            <i class="las la-plug"></i> @lang('Connect Extension')
                                        </button>
                                        <button type="button" class="btn btn--info btn--sm" id="geHistoryBtn">
                                            <i class="las la-history"></i> @lang('Load History')
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div class="col-lg-7">
                                    <div class="border rounded p-3">
                                        <h6 class="mb-2">@lang('Live Import Status')</h6>
                                        <p class="text-muted fs-14 mb-2" id="geStatusText">
                                            @lang('No active job. Click Connect Extension, then start extraction from WhatsApp Web.')
                                        </p>
                                        <div id="geJobPanel" class="d-none">
                                            <div class="row gy-2">
                                                <div class="col-sm-6"><strong>@lang('Job ID'):</strong> <span id="geJobId">-</span></div>
                                                <div class="col-sm-6"><strong>@lang('Status'):</strong> <span id="geJobStatus">-</span></div>
                                                <div class="col-sm-6"><strong>@lang('Total'):</strong> <span id="geTotal">0</span></div>
                                                <div class="col-sm-6"><strong>@lang('Processed'):</strong> <span id="geProcessed">0</span></div>
                                                <div class="col-sm-6"><strong>@lang('Imported'):</strong> <span id="geImported">0</span></div>
                                                <div class="col-sm-6"><strong>@lang('Updated'):</strong> <span id="geUpdated">0</span></div>
                                                <div class="col-sm-6"><strong>@lang('Skipped'):</strong> <span id="geSkipped">0</span></div>
                                                <div class="col-sm-6"><strong>@lang('Failed'):</strong> <span id="geFailed">0</span></div>
                                            </div>
                        ...
                        <div class="tab-pane fade" id="legacy-paste-pane" role="tabpanel">
                            <form action="{{ route('user.contact.import.group') }}" method="POST" id="import-group-form">
                                ...
                                <div class="d-flex justify-content-end gap-2 mt-3">
                                    ...
                                    <button type="submit" class="btn btn--base">@lang('Import Legacy Paste')</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
@push('script-lib')
    ...
    <script src="{{ asset('assets/global/js/group_extraction_client.js?v=20260221c') }}"></script>
@endpush
        ...
        (function($) {
            ...
            if (window.GroupExtractionClient) {
                window.GroupExtractionClient.init({
                    routes: {
                        session: "{{ route('user.contact.group.extraction.session') }}",
                        status: "{{ route('user.contact.group.extraction.status', ['id' => '__JOB_ID__']) }}",
                        result: "{{ route('user.contact.group.extraction.result', ['id' => '__JOB_ID__']) }}",
                        retry: "{{ route('user.contact.group.extraction.retry', ['id' => '__JOB_ID__']) }}",
                        history: "{{ route('user.contact.group.extraction.history') }}",
                    },
                    selectors: {
