    ...
    Route::middleware(['check.status', 'registration.complete'])->group(function () {
        ...
        Route::namespace('User')->group(function () {
            ...
            Route::controller('ContactController')->prefix('contact')->name('contact.')->group(function () {
                ...
                Route::controller('GroupExtractionController')->group(function () {
                    Route::post('group-extraction/session', 'createSession')->name('group.extraction.session')->middleware(['agent.permission:add contact', 'has.subscription']);
                    Route::get('group-extraction/history', 'history')->name('group.extraction.history')->middleware(['agent.permission:add contact', 'has.subscription']);
                    Route::get('group-extraction/jobs/{id}', 'status')->name('group.extraction.status')->middleware(['agent.permission:add contact', 'has.subscription']);
                    Route::get('group-extraction/jobs/{id}/result', 'result')->name('group.extraction.result')->middleware(['agent.permission:add contact', 'has.subscription']);
                    Route::post('group-extraction/jobs/{id}/retry-failed', 'retryFailed')->name('group.extraction.retry')->middleware(['agent.permission:add contact', 'has.subscription']);
                });
            });
            ...
            Route::get('report/{id}/analytics', 'reportAnalytics')->name('report.analytics')->middleware('agent.permission:view campaign');
